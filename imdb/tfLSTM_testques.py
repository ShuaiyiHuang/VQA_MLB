...
# imdb_data=tfloader.get_dataset(train_prefix, val_prefix, test_prefix, max_document_length)
imdb_data=tfimdbloader.load_imdb()
...
tfargs.max_doc_length=100

X_train,y_train,q_train,ques_train= imdb_data.train.images, imdb_data.train.labels, imdb_data.train.queries, imdb_data.train.ques
X_validation,y_validation,q_validation,ques_validation= imdb_data.val.images, imdb_data.val.labels, imdb_data.val.queries, imdb_data.val.ques
X_test,y_test,q_test,ques_test= imdb_data.test.images, imdb_data.test.labels, imdb_data.test.queries, imdb_data.test.ques
...
#########LSTM######################
imdb_preprocess.embedding_prepare(max_document_length=max_document_length,use_glove=True,trainable=True)

raw_embedded_chars=tfembedding.get_embedded_from_wordid(ques)
embedded_chars=tf.reshape(raw_embedded_chars,shape=[tfargs.batch_size,max_document_length,tfargs.embedded_dim])
...
with sess.as_default():
    ...
    saver.save(sess, '../../data/Models/0508/LSTM')
    print("LSTM Model saved")

with tf.Session() as sess:
    saver.restore(sess, tf.train.latest_checkpoint('../../data/Models/0508'))
