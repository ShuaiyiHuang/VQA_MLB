...
class VocabularyProcessor(object):
    ...
    def fit_vocab(self, raw_documents, unused_y=None):
        """Learn a vocabulary dictionary of all tokens in the raw documents.
    ...
    def transform_vocab(self, raw_documents):
        """Transform documents to word-id matrix.
    ...
    """
        for tokens in self._tokenizer(raw_documents):
            word_ids = np.zeros(self.max_document_length, np.int64)
            #pad word id with -1,zero vector added to embedding tensor
            word_ids=word_ids-1
            for idx, token in enumerate(tokens):
                if idx >= self.max_document_length:
                    break
                word_ids[idx] = self.vocabulary_.get(token) - 1  # get rid of the unknown token
            yield word_ids
