...
def load_imdb(path='../data',max_length=100):

    datapath=os.path.join(path,'imdb.pkl')
    dicpath=os.path.join(path,'imdb.dict.pkl')
    dic_set=load_dictionary(dicpath)

    tfargs.vocab_size=len(dic_set)
    print('imdb dict length:',len(dic_set))

    print(datapath)
    train,valid,test=load_data(datapath,n_words=100000,valid_portion=0.1,maxlen=max_length)
